...
// path instead of the full htmx refetch every ~5s tick would otherwise
...
(function () {
  ...
  // Match web/static/js/metrics.js's series palette / main.css tokens:
  // --color-primary for utilization, --color-status-running for memory.
  var UTIL_COLOR = "#2f5fda";
  var MEM_COLOR = "#1a8a5f";
  ...
  function applyRow(inst) {
    ...
    if (us) us.innerHTML = stripSVG(inst.gpuUtil, UTIL_COLOR);
    if (uv) uv.textContent = latestLabel(inst.gpuUtil);
    if (ms) ms.innerHTML = stripSVG(inst.gpuMem, MEM_COLOR);
  ...
  window.sparkyDashboardRenderAll = function (data) {
    ...
    data.instances.forEach(applyRow);
  };
  ...
  // instance_result event and this tick), fall through to a full-page
