...
// telemetry tick fetches this instead of triggering the full htmx page
// refetch instance_result still uses, so the strips advance in place
// without visibly redrawing the table. Same Read-only/no-audit posture as
// /dashboard itself - reads are never audited (ARCHITECTURE.md Audit Log) -
// and the same shape as the Metrics page's /metrics/chart-data.
