...
type Node struct {
	...
	// SSHPublicKey is this node's own peer-transfer client identity,
	// generated locally by `sparky-agent setup` and reported at connect
	// time - nil until an upgraded agent has connected at least once. The
	// private half never leaves the node - see
	// migrations/000028_add_nodes_ssh_identity.up.sql.
	SSHPublicKey *string
	// SSHHostPublicKey is this node's system sshd host key (OS-managed,
	// not generated by Sparky), reported so a peer pulling FROM this node
	// can pin trust for that one connection instead of blind TOFU.
	SSHHostPublicKey *string
	// DefaultTransferInterface names one of this node's own
...
// UpdateSSHIdentity records the SSH identity an agent reports at connect
// time (its own client public key, and its system sshd's host public key
// if it has one) - see SCHEMA.md Nodes' ssh_public_key/ssh_host_public_key.
// Not yet called by anything - agent/connection's Hello handshake doesn't
// report these fields yet (a later PR) - added now alongside the schema
// it writes to, same "built before it has a caller" shape several other
// pieces of this codebase already have (e.g. internal/profiles.Service.DeleteProfile
// before its own route existed).
func (r *NodeRepository) UpdateSSHIdentity(ctx context.Context, nodeID string, sshPublicKey, sshHostPublicKey *string) error {
	_, err := r.pool.Exec(ctx,
		`UPDATE nodes SET ssh_public_key = $1, ssh_host_public_key = $2 WHERE id = $3`,
		sshPublicKey, sshHostPublicKey, nodeID)
	if err != nil {
		return fmt.Errorf("update SSH identity for node %s: %w", nodeID, err)
	}
	return nil
}

// SetDefaultTransferInterface records an Admin's choice of this node's
// default network interface for peer-to-peer model transfers - see
// SCHEMA.md Nodes' default_transfer_interface. interfaceName nil clears
// it back to "Fastest" (auto-pick the highest reported link speed).
// Deliberately no FK to node_network_interfaces - see that table's own
// migration for why; a Go-level check against currently-reported
// interfaces belongs in the service layer that will call this
// (internal/nodes.Service, a later PR), not here.
func (r *NodeRepository) SetDefaultTransferInterface(ctx context.Context, nodeID string, interfaceName *string) error {
	_, err := r.pool.Exec(ctx,
		`UPDATE nodes SET default_transfer_interface = $1 WHERE id = $2`,
