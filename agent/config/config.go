...
type Config struct {
	...
	// SSHKeyPath / SSHHostKeyPath locate this node's own SSH client
	// public key (generated by `sparky-agent setup`) and its system
	// sshd's host public key, both read once at startup and reported in
	// the hello handshake for peer-to-peer model transfer -
	// SPARKY_SSH_KEY_PATH / SPARKY_SSH_HOST_KEY_PATH, optional. Each is the
	// *private*-key-adjacent .pub path; the agent only ever reads public
	// halves. A missing file is not an error - see provision.ReadSSHPublicKey.
	SSHKeyPath     string
...
// defaultSSHKeyPath / defaultSSHHostKeyPath duplicate agent/provision's
// exported defaults rather than importing it - agent/config has no
// dependencies on other agent packages, same precedent as the duplicated
// constants above. SPARKY_SSH_KEY_PATH names the private key's path (what
// setup generates); the public half is read from "<path>.pub".
const (
	defaultSSHKeyPath     = "/opt/sparky/serviceloop/.ssh/id_ed25519"
...
func Load() (*Config, error) {
	cfg := &Config{
		...
		SSHKeyPath:            getEnvDefault("SPARKY_SSH_KEY_PATH", defaultSSHKeyPath),
		SSHHostKeyPath:        getEnvDefault("SPARKY_SSH_HOST_KEY_PATH", defaultSSHHostKeyPath),
