...
|-------|----------|-------------------|
| CDI GPU passthrough via Podman's Docker-Engine-API-compatible socket not yet verified on target hardware/Podman version - neither Docker API mechanism for requesting a CDI device (`HostConfig.DeviceRequests` with `Driver: "cdi"`, or `HostConfig.Devices` with a CDI name as `PathOnHost`) triggered CDI resolution against a real local Podman 4.9.3 daemon, even though Podman's own CLI resolves CDI names correctly - see 2026-08-10 Decisions Log entry for the full empirical finding. Compounding this: `runtime.Spec.CDIDevices` is pure scaffolding today - nothing in production code (`agent/connection.go`'s Spec-building call site) actually populates it, so every real Docker/Podman GPU launch currently sends zero device requests over the Engine API regardless of mechanism | Medium | No longer a DGX Spark blocker as of 2026-08-19 - the fleet's Spark workstations ship with Docker and stay on it deliberately (matching NVIDIA's own Spark workflow assumptions), so Spark validates Docker's native GPU passthrough mechanism instead, tracked separately below. This row's actual remaining blocker is the Dell Precision RTX 3080Ti (Podman's intended validation target) not yet being online; `agent/runtime/containers` implements the documented, correct Docker API contract regardless, which is right for Docker and the best available attempt for Podman |
| Two fixes from comparing a real production vLLM launch script against Sparky's current support remain unimplemented (approved as a 4-part plan on 2026-08-17; two of the four are already done - unknown `engine_params` key rejection, and the per-profile container image override, see that date's and 2026-08-17's later Decisions Log entries): (1) GPU passthrough mechanism should branch by `runtime_backend` - the script proves Docker's native `DeviceRequests{Driver:"nvidia"}` mechanism (`--gpus all`) works on real production Spark hardware, a different Engine API shape from CDI, which Podman should keep as its own recommended mechanism; (2) no `--shm-size`/`--ipc=host` support, needed for the script's multi-GPU tensor-parallel launch | Medium | Confirmed approach for both exists in the approved plan (file-level detail available in that planning session); not yet implemented - deliberately sequenced as separate branches/PRs (one logical change per branch). As of 2026-08-19, fix (1)'s Docker-native branch is confirmed as the fleet's actual production path for every DGX Spark node, not just one option among several - Spark ships Docker deliberately, never Podman, so once implemented this fix can be confirmed directly against real Spark hardware already in hand, unlike the CDI row above (now a Dell Precision RTX 3080Ti blocker, unrelated to Spark). The GPU passthrough fix's own Docker/nvidia code path will be verifiable here via the existing `fakeDockerClient` request-shape assertions in `containers_test.go` regardless |
