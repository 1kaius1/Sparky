...
| Three fixes from comparing a real production vLLM launch script against Sparky's current support remain unimplemented (approved as a 4-part plan on 2026-08-17; the fourth, rejecting unknown `engine_params` keys, is already done - see that date's Decisions Log entry): (1) GPU passthrough mechanism should branch by `runtime_backend` - the script proves Docker's native `DeviceRequests{Driver:"nvidia"}` mechanism (`--gpus all`) works on real production Spark hardware, a different Engine API shape from CDI, which Podman should keep as its own recommended mechanism; (2) no way to override the container image per profile (`vllmImage` is a hardcoded constant; the script uses `nvcr.io/nvidia/vllm:26.06-py3`, not the community `vllm/vllm-openai` image); (3) no `--shm-size`/`--ipc=host` support, needed for the script's multi-GPU tensor-parallel launch | Medium | Confirmed approach for all three exists in the approved plan (file-level detail available in that planning session); not yet implemented - deliberately sequenced as separate branches/PRs (one logical change per branch), landing after the unknown-keys fix. The GPU passthrough fix's own Docker/nvidia code path will be verifiable here via the existing `fakeDockerClient` request-shape assertions in `containers_test.go`, but real-hardware confirmation remains blocked the same way the CDI row above already is |
