...
| The sidebar's `Audit log`, `Users & permissions`, and `Settings` links are shown to every authenticated viewer regardless of tier, same as every other nav link - a non-Admin who clicks any of them now gets a friendly "access denied" page (fixed - see 2026-08-16 Decisions Log) rather than raw JSON, but the link itself still isn't hidden, so a non-Admin can always see and click through to it | Low | The shared `pageData`/`render()` path every page uses has no notion of the viewer's tier today, only whether a session exists at all - conditionally hiding these nav links by tier would mean threading a tier lookup into *every* page's render call (an extra DB round trip each time, not just these three pages' own, since the sidebar is shared shell markup rendered on every page), a materially bigger change than the friendlier-page half, deliberately scoped out and confirmed with the user rather than bundled in unilaterally |
