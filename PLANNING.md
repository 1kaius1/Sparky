...
| `agent/telemetry.Collector`'s multi-GPU aggregation (utilization averaged, memory summed across every GPU `nvidia-smi` reports) remains unverified against a real multi-GPU node - every node available to develop against (the RTX 4090 laptop, the Dell Precision RTX 3080Ti) has exactly one GPU. Single-GPU CSV parsing and real `/proc/stat`/`/proc/meminfo` parsing are no longer part of this gap - see the 2026-08-17 Decisions Log entry closing that half against real hardware | Low | Requires a real multi-GPU node to close, which this project does not have access to; the aggregation logic itself is unit-tested against a fake command runner exercising a realistic two-line multi-GPU CSV shape, so the gap is specifically "does a real multi-GPU `nvidia-smi` invocation actually emit one CSV line per GPU, in the order assumed," not "is the aggregation math correct for the shape it's given" |
| Dashboard UI's Metrics page has never had its Chart.js rendering checked with real telemetry actually flowing through it - narrowed down from the previous, broader "never checked in a real browser at all" gap by the 2026-08-18 Decisions Log entry, which confirmed every page's static rendering (layout, CSS, real-DOM htmx swap behavior, sidebar/SSE persistence across navigation) directly in a real browser. What's left is specifically whether `initMetricsChart` draws a correct, readable chart once real data is streaming in via SSE - that pass could only exercise the chart's empty/no-data state, since generating real telemetry needs a live agent connection to a real GPU node | Low | Needs the RTX 4090 laptop already on record as this project's primary GPU dev/test hardware (Dependencies and Blockers below), which was in use for unrelated work during the 2026-08-18 pass - not blocked on missing tooling this time, unlike the rest of this table |
