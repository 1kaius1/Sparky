...
- **Local-only accounts, created by the SuperAdmin, participating in the existing
  RBAC tier system** - not trivial, but a real, confirmed need: this project's own
  primary deployment target has no AD/LDAP infrastructure at all, and standing one
  up solely to run Sparky itself is explicitly not something the user wants to do.
  Today, auth is entirely AD-backed - every real `Users` row is created by a
  successful LDAP bind, `SCHEMA.md` Users' `ad_sid` is unique and indexed, and the
  session-recheck mechanism (`AUTH_RECHECK_INTERVAL_SECONDS`) periodically
  re-verifies login-gate group membership against LDAP for the life of a session -
  none of which has any meaning for an account with no directory behind it at all.
  Two existing mechanisms are the most relevant prior art to design against, not
  starting from nothing: the `IdentityProvider` interface (`internal/auth`,
  `ARCHITECTURE.md` Auth & Identity Provider) already exists specifically so a
  second backend can sit behind "authenticated user + resolved group
  memberships" without touching anything downstream - though it was designed with
  OIDC/Entra ID in mind (another *external* directory), not a purely local
  account with no directory at all, so whether local accounts fit that same
  interface shape or need their own mechanism is a real open question, not
  resolved here; and the Break-glass credential (`SCHEMA.md`) already establishes
  the precedent for a locally-stored, Argon2id/bcrypt-hashed credential outside
  AD entirely, plus the precedent that AD-recheck simply does not apply to a
  non-AD-backed session (break-glass sessions are already never rechecked).
  Real open questions for whenever this is actually designed: whether local
  accounts get their own table or new nullable columns on `Users` (mirroring
  `ad_sid`/`ldap_dn`'s already-nullable, AD-specific shape); how a SuperAdmin
  actually creates one (a CLI subcommand, matching `sparky-server setup`/
  `set-superadmin-password`'s existing "sensitive account operations are
  CLI-only" precedent, or a UI form - a real design fork, not decided here);
  whether a local account's tier is set at creation time or still goes through
  the normal elevation rules afterward; and how login itself distinguishes a
  local-account attempt from an AD-bind attempt at the `/login` form (a
  separate local-login route/form, or one form that tries both).
