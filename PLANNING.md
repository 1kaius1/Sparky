...
| Dashboard UI's Metrics page chart has real telemetry flowing through it end to end and, as of 2026-08-20, a hover crosshair and a bounded/right-justified display window (see that date's Decisions Log entry) - but that same fix has not yet itself been confirmed in a real browser: whether the crosshair actually tracks the cursor, whether the chart genuinely stops growing past the last hour, and whether a sparse series visibly right-justifies as designed are all purely visual claims this sandboxed environment's own `curl`/`psql`-based verification cannot confirm | Low | Needs the user's own look at the rebuilt server in a real browser - not blocked on missing tooling or hardware, just on a human glancing at the page after this fix's server restart and a static-asset hard-refresh |
