...
| Metrics page chart, confirmed in a real browser for the first time 2026-09-08 (closing the prior "never visually confirmed" gap below) against two real, concurrently-registered Sparks: each node's series renders as its own contiguous block along the shared x-axis instead of overlaying properly against a common timeline - Spark-1's line occupies roughly the left half of the panel, Spark-2's the right half, abutting rather than interleaved, even though both are correctly plotted as distinct colored series in one shared canvas/legend (the one-graph-multiple-lines design itself is not broken - see the 2026-09-08 Decisions Log entry for the real screenshot evidence and root-cause read of `web/static/js/metrics.js`) | Medium | Root cause identified but not yet fixed: `initMetricsChart`'s x-axis is a Chart.js `category` scale, and `fitSeriesToWidth` pads/right-justifies each series independently using each node's own literal timestamp strings as category labels - since two independently-connected agents poll telemetry on their own 5-second intervals with no shared clock alignment, their label sets rarely coincide, so the category scale lays out the union of both series' distinct labels in first-seen order, producing two abutting blocks rather than one interleaved timeline. The code's own existing comment on `fitSeriesToWidth` anticipated imperfect tick-for-tick alignment as an accepted trade-off, but undersold the real visual impact once actually seen with two real nodes. Needs a real design decision (e.g. a shared/binned time axis instead of an independent-per-series category axis) before implementing, not a quick patch |
