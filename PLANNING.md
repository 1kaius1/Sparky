...
| 2026-09-10 | Scoped SSE-driven live refreshes to the page that actually shows the changed data, and added a form-control focus guard - closing a real GUI bug found during manual end-to-end testing (see `FIX_REFRESH.md` for the full assessment). Before: `web/static/js/sse.js` wired `transfer_progress` / `engine_transfer_progress` / `instance_result` events straight to `scheduleRefresh`, an unconditional full `#main-content` htmx refetch of whatever page happened to be open, for any such event anywhere in the fleet - so a periodic `transfer_progress` tick (progress is streamed, not just terminal, per `docs/AGENT.md`) would destroy and rebuild a create/edit form the user was part-way through, closing its open `<select>` within the 500ms debounce window. After: each list page's `content`-block wrapper carries a `data-sse-topics` attribute naming the event types whose data it displays (`transfers` -> `transfer_progress`; `engine_transfers`/`engine_inventory` -> `engine_transfer_progress`; `dashboard` -> `instance_result`; `profiles` -> `instance_result instance_health`); every other page - all six create/edit forms, plus `nodes`/`audit`/`users`/`settings`/`node_registered`/`forbidden` - has no wrapper and so never auto-refreshes. `sse.js` gained `pageWantsTopic()` (reads that attribute, "not present" = wants nothing, the safe default), a fourth listener for `instance_health` (published server-side since the health-check work but never listened for in the browser until now - the Profiles Health column now updates live), and a focus guard in `scheduleRefresh` that drops (does not re-queue) a refresh landing while `document.activeElement` is an `INPUT`/`SELECT`/`TEXTAREA` inside `#main-content`. The `telemetry` path is untouched - it already self-scopes via `sparkyMetricsLiveUpdate`'s canvas-presence check | The five decisions behind this were taken with the user one at a time (see `FIX_REFRESH.md`'s Decisions section): (1) per-page `data-sse-topics` on a wrapper `<div>`, chosen over a JS `pathname`->topics map or a central Go map + response header - it keeps the declaration next to the page and needs no server change, and is layout-safe (`.main` is a flex item with no `> *` child selectors in `main.css`, confirmed); (2) fix the `instance_health` listener gap in the same pass rather than filing it; (3) `engine_inventory` opts in to `engine_transfer_progress` despite the mostly-redundant intra-run ticks, since that is the only signal a run has completed; (4) the node online/offline live-update gap is filed as its own Known Issues row, not fixed here (needs a new server-side event); (5) focus guard only, no `hx-preserve` - topic scoping alone already fixes the reported bug (the form pages it happened on now declare no topics), the guard is forward-looking defence-in-depth for the list pages. Verified: `go build ./... && go vet ./... && gofmt -l . && go test ./...` clean (the `internal/httpapi` template-loading tests parse every page template, so the wrapper-div edits are exercised). Real-browser confirmation against a live fleet is the remaining manual gate, same precedent as the metrics-chart entries above | A JS `window.location.pathname`->topics lookup table in `sse.js` (rejected - puts route knowledge in a second place and needs fiddly prefix rules to tell `/profiles` the list from `/profiles/new` the form); a central page->topics map in `render.go` delivered via a `pageData` field for full loads plus a response header for htmx swaps (rejected - single source of truth is nice but it needs two delivery mechanisms and an `htmx:afterSettle` hook for what a one-line attribute in each of five templates does directly); per-node/per-resource-ID event matching (rejected for now - more precise, but a create form has no relationship to any specific transfer regardless of node, so form pages still need excluding outright either way; kept as a future refinement if a list page ever needs to ignore individual irrelevant rows within a topic it otherwise wants) |
| 2026-09-10 | Rebuilt the Dashboard's instance section: the old capped 10-row "Recent running instances" table (Status / Node / Started) is replaced by a "Running instances" table - one row per non-terminal instance (`running` / `starting` / `stopping`) showing model (joined from the profile), engine type, node, started time, a coarse humanized uptime, status + health, and two per-row inline-SVG "load strips" (recent GPU utilization % and GPU memory %). Prompted by the user doing hands-on fleet operation and finding the dashboard too thin to answer "what is running and how hard is it working" without opening Metrics | Design settled with the user one decision at a time, including a rendered static HTML mock-up to choose the strip layout (`internal/httpapi/dashboard.go`, `web/templates/pages/dashboard.html`, new `web/static/js/dashboard.js`, `web/static/css/main.css`, route in `router.go`): (1) replace, not augment, the old table; (2) strips are hand-rolled inline SVG bars, not Chart.js instances - non-interactive, cheap to redraw, no per-row chart lifecycle; (3) two separate columns ("GPU util" / "GPU mem"), each a strip plus a numeric readout of the latest value - the user chose this over one stacked "Load" column after seeing both mocked up, reads fine even at 720p; (4) ~24 bars ~= 2 min at the 5s telemetry cadence (`dashboardLoadBars`). Live-update mirrors the Metrics page exactly rather than inventing a new mechanism: new `GET /dashboard/live-data` returns the same JSON shape the page embeds as an initial seed (like `metrics.html`'s `chartData`); `dashboard.js` exposes `window.sparkyDashboardLiveUpdate` / `sparkyDashboardRenderAll`, both self-scoping (no-op without `#dashboard-running`); `sse.js`'s `telemetry` listener was generalized from "call `sparkyMetricsLiveUpdate` if defined" to "call every registered in-place updater" so a tick drives Metrics charts and Dashboard strips without a full-page refetch. Memory is shown as `used/total*100` percent (the strips are a "how loaded" indicator; absolute MB and full history stay on Metrics). Reused: `buildMetricsChartData`'s grouping shape, `handleMetricsChartData`'s endpoint pattern, the existing `a.metrics.ListRecentGPU` (last hour of GPU rows - the same query the Metrics page already runs every tick, so no new load pattern) trimmed to the newest `dashboardLoadBars` per instance in `buildDashboardLiveData`. Verified: `go build ./... && go vet ./... && gofmt -l . && go test ./internal/httpapi/...` clean; new unit tests for `humanizeUptime`, `buildDashboardLiveData` (grouping, 24-cap keeps newest, mem %, nil-correlation skipped, non-live statuses excluded, multi-GPU averaged), the handler (model/engine/uptime/`data-instance-id`/seed present, stopped instance excluded, dependency-failure 500s), and `/dashboard/live-data` (JSON shape, `RequireSession`). Real-browser confirmation against the live fleet is the remaining manual gate | Adding a dedicated short-window `metrics.Service` method for the strips instead of reusing `ListRecentGPU` (considered - would fetch fewer rows per tick; rejected as premature: the Metrics page already runs the identical hour-window query every ~5s, so this adds no new load profile, and a second window constant is easy to add later if it matters); one combined strip (bar height = util, colour = memory pressure) or one stacked "Load" column (both mocked up and rejected by the user in favour of two columns); Chart.js mini-instances per row (rejected - N chart instances with per-swap teardown for a low-fidelity indicator) |
...
| The Nodes page and the Dashboard "Online" stat tile do not live-update when a node's agent connects or disconnects - a node going offline (or coming back) is only reflected after a manual navigation. Agent connect/disconnect is handled on the WebSocket lifecycle in `internal/agentconn` / `agentconn.Registry`, which is not one of `cmd/sparky-server/main.go`'s `onMessage` five message types, and nothing publishes an `events.Event` for it, so `web/static/js/sse.js` has no signal to refresh on | Low | Found while scoping the 2026-09-10 SSE-refresh fix (see that date's Decisions Log entry), not fixed there - it needs new server-side work (a new `events.Event` type or reuse of an existing one, plus a `Publish` call at the connect/disconnect points in `agentconn`), which is out of proportion to a refresh-scoping change. Once added, `nodes.html` and `dashboard.html` would declare that topic in their `data-sse-topics` wrapper the same way the other list pages already do |
