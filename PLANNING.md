...
| Docker's GPU passthrough now sets both `DeviceRequests{Driver: "nvidia"}` and `NVIDIA_VISIBLE_DEVICES=all` in the container env, matching the one real production launch script this mechanism is confirmed against (which sets both explicitly alongside `--gpus all`) - whether the env var is still required alongside the modern `DeviceRequests` API, or purely redundant with it, could not be confirmed in this environment | Low | Needs a real Docker/GPU launch with the env var deliberately omitted to determine if it changes anything - not attempted, since removing it without evidence either way risks silently breaking something `DeviceRequests` alone doesn't cover; kept for parity with the only real evidence available rather than guessed away |
