...
|---|---|---|
| v0.1.0 - Core foundation | Substantially complete | Docker runtime-backend GPU passthrough validation against real DGX Spark hardware, now in hand - CDI/Podman validation specifically moved off Spark entirely (see 2026-08-19 Decisions Log entry) and now waits on the Dell Precision RTX 3080Ti, not yet online |
| v0.2.0 - Bare-metal backend + Spark validation | Well underway | Same Docker runtime-backend validation as v0.1.0 |
...
- [ ] **v0.2.0** - Bare-metal runtime backend, and Spark hardware validation
  ...
  - [ ] Validate the Docker runtime backend's native GPU passthrough
        (`DeviceRequests{Driver:"nvidia"}`, `--gpus all` - not CDI) against
        real DGX Spark hardware, now in hand. The GB10 GPU supports
        passthrough to a container without affecting a display connected to
        it (NVIDIA's supported use case for that hardware), unlike the dev
        laptop. Confirmed 2026-08-19: the fleet's Spark workstations ship
        with Docker and stay on it deliberately - no desire to switch to
        Podman there, matching NVIDIA's own Spark workflow assumptions - so
        CDI/Podman is not this item's path for Spark at all; see that date's
        Decisions Log entry
...
| The v0.1.0/v0.2.0 checklist item "Validate the Docker runtime backend's native GPU passthrough... against real DGX Spark hardware" is still open. A first real-hardware validation attempt (2026-08-20 Decisions Log) found and fixed a real `buildEngineLaunchArgs` bug before the container ever started successfully, so no completed end-to-end pass (real container up, real inference served, GPU utilization confirmed non-zero) exists yet - only unit/dispatch-level test coverage of the fix itself | Medium | Needs a re-run of the corrected validation script against the real Spark hardware, now that the blocking bug is fixed. Not blocked on hardware access (already in hand and actively used) - just on completing the run |
...
- Real DGX Spark hardware is needed for v0.2.0's Docker runtime-backend GPU
  ...
  hardware is in hand and being actively used, so this line's remaining
  blocker is the validation work itself, not access. Also as of 2026-08-19,
  this is a Docker-only item, not "Docker/Podman" as previously stated here
  - the fleet's Spark workstations ship with Docker and stay on it
  deliberately, matching NVIDIA's own Spark workflow assumptions, with no
  desire to switch to Podman there. CDI/Podman validation itself moves
  entirely to the Dell Precision RTX 3080Ti bullet above instead, since
  Spark will never run Podman - that bullet's "already in hand" describes
  physical possession, not that the machine is currently racked/configured/
  online for active testing the way the RTX 4090 laptop is; per the user
  directly, Podman/CDI validation specifically waits until the Dell
