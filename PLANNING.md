...
| `containers.go:123` sets `DeviceRequests.Driver` to the literal string `"nvidia"`, and its own doc comment claims this is "the same request `docker run --gpus all` makes" - but a real `docker run --gpus all` on real DGX Spark hardware (Docker Engine 29.2.1, confirmed via `docker inspect` against both the live production container and a 2026-08-20 validation test container) actually produces `Driver: ""` (empty string), not `"nvidia"`. It evidently works either way on this daemon (the empty-string production container is serving real inference), so this is likely benign, but neither real container observed so far has actually exercised Sparky's literal `"nvidia"` value - only Docker CLI's own shorthand has been confirmed working | Low | Needs a request built via the raw Docker Engine API (bypassing `docker run --gpus all`'s CLI translation) with the exact JSON body `containers.go` constructs, to confirm `Driver: "nvidia"` specifically also works on this daemon - not yet attempted, since the CLI-based validation script above already surfaced enough to need a re-run for other reasons first |
