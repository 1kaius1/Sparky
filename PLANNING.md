...
| Last of the four fixes from comparing a real production vLLM launch script against Sparky's current support remains unimplemented (approved as a 4-part plan on 2026-08-17; the other three are done - unknown `engine_params` key rejection, the per-profile container image override, and the GPU passthrough mechanism branching by `runtime_backend`, see that date's and 2026-08-17's/2026-08-19's later Decisions Log entries): no `--shm-size`/`--ipc=host` support, needed for the script's multi-GPU tensor-parallel launch | Medium | Confirmed approach exists in the approved plan (file-level detail available in that planning session); not yet implemented - sequenced last since it reuses `containers.go`'s/`LaunchSpec`'s regions the GPU passthrough fix just touched. Its own request-shape (`HostConfig.ShmSize`, `HostConfig.IpcMode`) will be verifiable here via the existing `fakeDockerClient` capture pattern in `containers_test.go`, same as every other container-level fix in this series - real-hardware tuning confirmation (is `64g`+`host` actually right for a given workload) remains a separate, likely-permanent gap needing real GPU hardware under real multi-GPU load |
