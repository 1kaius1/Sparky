...
| Dashboard UI's Metrics page chart has been confirmed end-to-end with real telemetry actually flowing through it (real `gpu_metrics` rows, real `/metrics/chart-data` JSON, real server-rendered page - see the 2026-08-18 Decisions Log entry) up to the point a human looks at the rendered chart in a browser - whether it visibly avoids redrawing on a live tick, and whether the hover tooltip renders correctly, are purely visual claims this sandboxed environment's own `curl`/`psql`-based verification cannot confirm | Low | Needs the user's own look in the real browser already opened against this pass's running server - not blocked on missing tooling or hardware, just on a human glancing at the page |
