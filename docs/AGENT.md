...
| `SPARKY_SSH_KEY_PATH`            | No       | `/opt/sparky/serviceloop/.ssh/id_ed25519` | This node's own SSH client private key (generated by `sparky-agent setup`) - only the adjacent `.pub` is ever read and reported. See SSH identity below |
| `SPARKY_SSH_HOST_KEY_PATH`       | No       | `/etc/ssh/ssh_host_ed25519_key.pub` | The node's system sshd host public key, reported so a peer can pin trust instead of trust-on-first-use. Not universal across distros - override if yours differs. A missing file is fine (a destination-only node needs no sshd) |
| `SPARKY_PEER_TRANSFER_AUTH_TTL_SECONDS` | No | `7200` | How long a source-side peer-transfer authorization stays usable if never revoked; non-positive means the default. See Peer-to-peer model transfer below |
...
the private key never leaves the node. The central app validates each
reported key is a bare `<type> <base64>` line before storing it.

Right after every successful handshake the agent also sends `report_interfaces`
- each up, non-loopback interface with a usable address and its link speed
from `/sys/class/net/<name>/speed` (nil when unknown, never 0), skipping
container/VM plumbing (`docker*`, `veth*`, `br-*`, ...). A `rescan_interfaces`
...
pulled by the destination. The full threat model - this is the one scoped
exception to "no inbound network exposure" - is in `ARCHITECTURE.md`
Security Considerations; this section is the operator view.

`sparky-agent setup` prepares a node to be a *source*: it creates the
`sparky-peer` account (locked password, empty home `/var/lib/sparky-peer`, real
`/bin/sh` because sshd runs a forced command through the account's shell, member of
`serviceloop`'s group so it can read downloaded models), the grant directories
`/opt/sparky/serviceloop/peer-grants` and `peer-used`, and an sshd drop-in
`/etc/ssh/sshd_config.d/50-sparky-peer.conf` whose only content is a
`Match User sparky-peer` block (public-key only, no TTY, no forwarding, keys supplied
by `sparky-agent peer-authkeys`). It runs `sshd -t` and rolls the drop-in back if sshd
rejects it, then reloads sshd if systemd is present. It fails with a clear error if
the host's `sshd_config` has no `Include /etc/ssh/sshd_config.d/*.conf`; a host that
restricts logins with `AllowUsers`/`AllowGroups` must also allow `sparky-peer`.
Model storage must be readable by `serviceloop`'s group - the default location is;
a custom `SPARKY_MODEL_STORAGE_PATH` elsewhere needs the same.

Per transfer, the central app asks the source agent to authorize the destination's
public key; the agent writes a grant file, and sshd computes one restricted
authorized_keys line from each live grant on demand. A grant works for exactly one
connection, expires after `SPARKY_PEER_TRANSFER_AUTH_TTL_SECONDS`, and is revoked when
the transfer ends. The destination pins the source's sshd host key. A node that has not
re-run `setup` since this shipped refuses a source role cleanly (the source reports the
reason back), and a node without sshd can still be a destination.
